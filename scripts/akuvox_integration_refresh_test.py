...
Example:
    python3 scripts/akuvox_integration_refresh_test.py --country-code 1 --phone 2122561078 --subdomain ucloud --sms-code 123456
...
async def _run_flow(
    *,
    ...
    try:
        ...
        await client._data.async_set_stored_data_for_key("refresh_token", client._data.refresh_token)  # type: ignore[attr-defined]
...
def main() -> int:
    ...
    parser.add_argument("--phone", required=True, help="Plain phone number (digits only).")
    parser.add_argument("--subdomain", required=True, help="Akuvox deployment subdomain.")
    parser.add_argument("--sms-code", required=True, help="SMS verification code from Akuvox.")
    parser.add_argument(
        "--refresh-count",
        type=int,
        default=3,
