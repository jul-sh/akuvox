...
PLATFORMS: list[Platform] = [Platform.CAMERA, Platform.BUTTON, Platform.SENSOR]
...
async def async_update_configuration(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    try:
        if entry.options:
            ...
            # Wait for image URL?
            updated_options["wait_for_image_url"] = bool(
                updated_options.get("event_screenshot_options", "") == "wait"
            )
...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    async def async_update_tokens_service(call):
        ...
        if not entry_id or not token:
            LOGGER.error(
                "❌ Service call missing required parameters: entry_id and token"
            )
            return
        ...
        try:
            # Update tokens
            old_token = (
                client._data.token[:10] + "..."
                if len(client._data.token) > 10
                else client._data.token
            )
            ...
            if await client.async_retrieve_user_data():
                ...
                LOGGER.warning(
                    "⚠️  Token validation failed - unable to retrieve user data"
                )
    ...
    hass.services.async_register(
        DOMAIN, "update_tokens", async_update_tokens_service, schema=None
    )

    hass.services.async_register(
        DOMAIN, "refresh_tokens", async_refresh_tokens_service, schema=None
    )
